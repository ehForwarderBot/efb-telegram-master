...
class ChatBindingManager:
    ...
    def __init__(self, channel: 'TelegramChannel'):
        ...
        # Link handler
        self.bot.dispatcher.add_handler(CommandHandler("link", self.link_chat_show_list, pass_args=True))
        ...
        # Chat head handler
        self.bot.dispatcher.add_handler(CommandHandler("chat", self.start_chat_list, pass_args=True))
    ...
    def link_chat_show_list(self, bot, update, args=None):
        ...
        self.link_chat_gen_list(bot, update.message.from_user.id, pattern=" ".join(args))
    ...
    def link_chat(self, update, args):
        ...
        if slave_channel in coordinator.slaves:
            if chat.muted:
                chat.unlink()
            chat.link(self.channel.channel_id, update.message.chat.id, self.channel.flag("multiple_slave_chats"))

            txt = "Chat '%s' is now linked." % chat_display_name
            self.bot.send_message(update.message.chat.id, text=txt)
    ...
    def unlink_all(self, bot, update):
        ...
        links = self.db.get_chat_assoc(master_uid=utils.chat_id_to_str(self.channel.channel_id, update.message.chat.id))
        if len(links) < 1:
            return self.bot.send_message(update.message.chat.id, "No chat is linked to the group.",
                                         reply_to_message_id=update.message.message_id)
        else:
            self.db.remove_chat_assoc(master_uid=utils.chat_id_to_str(self.channel.channel_id, update.message.chat.id))
            return self.bot.send_message(update.message.chat.id,
    ...
    def get_chat_from_db(self, channel_id: str, chat_id: str) -> Optional[EFBChat]:
        ...
        if d:
            ...
            chat.chat_type = d.slave_chat_type
            return chat
        else:
